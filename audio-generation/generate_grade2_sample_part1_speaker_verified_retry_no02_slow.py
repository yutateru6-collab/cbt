...
OUTPUT_DIR = Path(
    "audio-generation/grade2-sample-part1-speaker-verified-retry-no02-slow-20260724"
)
BATCH_DISPLAY_NAME = "grade2-sample-part1-speaker-verified-retry-no02-slow-20260724"


def slow_prompt_for(item):
    transcript = "\n".join(
...
Speak every sentence itself at a relaxed, unhurried pace that is noticeably slower than ordinary conversation.
Do not create long extra pauses to imitate a slower pace; the spoken words themselves must be slightly slower.
Aim for the complete recording to last approximately {item['targetDurationSeconds']:.1f} seconds, including the requested pauses.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Retry only Part 1 No.2 with slower spoken sentences."
    )
...
verified.TARGET_WORDS_PER_SECOND = 2.38
verified.prompt_for = slow_prompt_for
...
verified.batch.prompt_for = slow_prompt_for
