...
OUTPUT_DIR = Path(
    "audio-generation/grade2-sample-part1-five-gemini-spacing-244-batch-20260724"
)
BATCH_DISPLAY_NAME = "grade2-sample-part1-five-spacing-244-20260724"
SOURCE_MANIFEST = "audio-generation/grade2-sample-part1-full.json"
VOICE_PAIR = {"A": "Kore", "B": "Puck"}
NUMBERS = range(1, 6)
TARGET_SPOKEN_WORDS_PER_SECOND = 2.44
...
Aim for an average spoken pace of approximately {TARGET_SPOKEN_WORDS_PER_SECOND:.2f} words per second, excluding the two requested pauses.
...
def build_plan():
    ...
    return {
        ...
        "paceInstructions": [
            (
                f"Aim for approximately {TARGET_SPOKEN_WORDS_PER_SECOND:.2f} "
                "spoken words per second, excluding requested pauses."
            ),
            *batch.PACE_INSTRUCTIONS,
        ],
        "paceTarget": {
            "spokenWordsPerSecond": TARGET_SPOKEN_WORDS_PER_SECOND,
            "measurementNote": (
                "final whole-file words per second will be slightly lower because "
                "it includes the two deliberate pauses"
            ),
        },
        "pauseInstructions": {
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Regenerate sample Grade 2 Part 1 No.1-5 at about 2.44 spoken "
            "words per second with slightly longer label pauses."
        )
    )
