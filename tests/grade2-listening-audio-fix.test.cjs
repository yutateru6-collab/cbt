...
function frameCount(buffer) { return buffer.readUInt32LE(40) / 2; }
...
async function main() {
  ...
  assert.equal(threeSet.GRADE2_LISTENING_SET01_DUPLICATE_QUESTION_FIX_V2_RELEASE, DUPLICATE_FIX_RELEASE);
  ...
  const q5Fixed = legacy.fixGrade2Set01ListeningWav(q5.buffer.slice(q5.byteOffset, q5.byteOffset + q5.byteLength), 5);
  ...
  const q9Fixed = legacy.fixGrade2Set01ListeningWav(q9.buffer.slice(q9.byteOffset, q9.byteOffset + q9.byteLength), 9);
  ...
  vm.runInNewContext(overrideSource, { window: { scbtGrade2VocabSets: sets }, Set }, { filename: "grade2-listening-set01-audio-fixes.js" });
  ...
  assert.match(examHtml, /grade2-set03-no11-travel-guide-gap-20260821-v1/);
  ...
  assert.match(workerSource, /GRADE2_LISTENING_SET01_DUPLICATE_QUESTION_FIX_V2_RELEASE/);
  assert.match(workerSource, /fixGrade2Set01DuplicateQuestionV2FromOneSecondWav/);
  assert.match(workerSource, /precomputedOnly: true/);
  ...
  console.log("grade2 listening ten-target, Set 03 No.11 gap-fix, No.5-only, and exactly-six V2 routing tests passed");
}
