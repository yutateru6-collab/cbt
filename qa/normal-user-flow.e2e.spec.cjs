...
async function finishQaMedia(page, cycles = 5) {
...
async function captureState(page, report, stateName, { fullPage = false } = {}) {
  ...
  report.states.push({ name: stateName, url: page.url(), title: await page.title(), metrics, screenshotFiles: files });
  report.screenshotFiles.push(...files);

  if (metrics.horizontalOverflow) addFinding(report, { severity: 'high', category: 'layout', state: stateName, detail: 'ページ全体で横スクロールが発生しています。' });
  if (metrics.overflowingElements.length) addFinding(report, { severity: 'medium', category: 'layout', state: stateName, detail: `画面外にはみ出す要素を${metrics.overflowingElements.length}件検出しました。` });
  if (metrics.clippedText.length) addFinding(report, { severity: 'low', category: 'text-clipping', state: stateName, detail: `切り詰め候補のテキスト要素を${metrics.clippedText.length}件検出しました。` });
  if (report.device.startsWith('iphone') && metrics.smallTargets.length) addFinding(report, { severity: 'low', category: 'touch-target', state: stateName, detail: `44×44px未満の操作要素を${metrics.smallTargets.length}件検出しました。` });
}
...
async function verifyProductionAudio(request, report, label, url) {
  ...
  const probe = { label, url, status: response.status(), contentType: response.headers()['content-type'] || '' };
  report.audioProbes.push(probe);
  expect([200, 206], `${label} audio HTTP status`).toContain(probe.status);
  expect(probe.contentType.toLowerCase(), `${label} audio content type`).toContain('audio');
}
...
async function runSpeakingExam(page, report) {
  ...
  for (let guard = 0; guard < 140; guard += 1) {
    ...
    const status = String(await page.locator('.speaking-live-status').getAttribute('class').catch(() => ''));
    if (stage && stages.at(-1) !== stage) stages.push(stage);

    for (const [pattern, name] of [[/Read Aloud/i, 'speaking-read-aloud'], [/^No\. 2$/i, 'speaking-no-2'], [/^No\. 4$/i, 'speaking-no-4']]) {
    ...
    const yes = page.locator('[data-action="grade2-speaking-choice"][data-choice="yes"]');
    ...
    if (await begin.isVisible().catch(() => false)) {
      await begin.evaluate((element) => element.click());
      await page.clock.fastForward(400);
      continue;
    }

    if (status.includes('counting')) await page.clock.fastForward(65000);
    else if (status.includes('recording')) await page.clock.fastForward(250);
    else await page.clock.fastForward(1500);
    await page.waitForTimeout(20);
  }

  report.speakingStages = stages;
...
async function driveListening(page, request, report) {
  ...
  for (let expectedId = 1; expectedId <= 30; expectedId += 1) {
    ...
    if ([1, 16, 30].includes(expectedId)) await captureState(page, report, `listening-no-${expectedId}`);
    ...
    await page.clock.fastForward(10200);
    await page.waitForTimeout(20);
  }
...
async function answerVisibleReading(page) {
  const ids = await page.locator('[data-action="written-answer"]').evaluateAll((elements) => [...new Set(elements.filter((element) => {
    const rect = element.getBoundingClientRect();
    return rect.width > 0 && rect.height > 0;
  }).map((element) => element.dataset.question).filter(Boolean))]);
  for (const id of ids) await page.locator(`[data-action="written-answer"][data-question="${id}"]`).first().click();
...
async function driveReading(page, report) {
  ...
  for (let guard = 0; guard < 40; guard += 1) {
    ...
    if (visited.size <= 2) await captureState(page, report, 'reading-first');
    if (!emailCaptured && ids.some((id) => id >= 24 && id <= 26)) {
      emailCaptured = true;
      await captureState(page, report, 'reading-email');
    }
    if (!longCaptured && ids.some((id) => id >= 27)) {
      longCaptured = true;
    ...
    if (!persistenceChecked && ids.length) {
      const id = ids[0];
      await next.click();
      await page.locator('[data-action="reading-prev"]').click();
      await expect(page.locator(`[data-action="written-answer"][data-question="${id}"].selected`).first()).toBeVisible();
  ...
  report.readingQuestionIds = [...visited].sort((a, b) => a - b);
...
async function driveWriting(page, report) {
  for (const [index, answer] of [summaryAnswer, essayAnswer].entries()) {
    const box = page.locator('textarea.writing-textarea').first();
    await expect(box).toBeVisible();
    await typeWritingAnswer(box, answer);
  ...
  await expect(page.locator('.result-screen.grade2-result-shell')).toBeVisible({ timeout: 10000 });
}

async function inspectResultAndReview(page, report, { restart = false } = {}) {
  const tabs = page.locator('[data-grade2-result-tab]');
  await expect(page.locator('.grade2-result-skill-tabs')).toBeVisible();
  ...
  for (const skill of ['reading', 'listening', 'writing', 'speaking']) {
    ...
    await captureState(page, report, `result-${skill}`, { fullPage: skill === 'listening' });
  }

  await page.locator('[data-grade2-result-tab="listening"]').click();
  const all = page.locator('[data-grade2-result-filter="all"][data-grade2-result-filter-skill="listening"]');
  ...
  await expect(page.locator('.result-screen.grade2-result-shell')).toBeVisible();
  await expect(page.locator('.grade2-result-skill-tabs')).toBeVisible();
...
for (const setKey of setKeys) {
  test(`normal paid user completes ${setKey} without dev mode`, async ({ page, request }, testInfo) => {
    ...
    try {
      ...
      report.actions.push('Complete Speaking with normal controls, synthetic QA microphone input, and accelerated timers only.');
      ...
      report.actions.push('Traverse all 31 Reading questions and verify answer persistence with real Previous/Next controls.');
      await driveReading(page, report);

      report.actions.push('Type both Writing answers and finish with the real confirmation modal.');
      await driveWriting(page, report);

      report.actions.push('Exercise all four result tabs, Listening direct replay/return, and Set 01 restart.');
      ...
      if (report.actionableRequestFailures.length) {
        addFinding(report, { severity: 'medium', category: 'network', state: 'whole-flow', detail: `無視できない通信失敗を${report.actionableRequestFailures.length}件検出しました。` });
      }
      ...
      writeJsonAtomic(path.join(partRoot, `${safeName(testInfo.project.name)}-${safeName(setKey)}.json`), report);
    }
  });
}
